...
Glib::RefPtr<Gdk::Pixbuf> RawImageHelper::getPixbuf()
{
#ifdef DEBUG_PREVIEW
    std::cout << "RawImageHelper::getPixbuf Converted RAW image size: "
              << m_memimg->width << " x " << m_memimg->height << std::endl;
#endif
    Glib::RefPtr<Gdk::Pixbuf> pixbuf;
    if (m_memimg->bits == 8 && m_memimg->colors == 3 && m_memimg->type == 2) {
        pixbuf = Gdk::Pixbuf::create(Gdk::COLORSPACE_RGB, false, 8,
                                     m_memimg->width, m_memimg->height);
#ifdef DEBUG_PREVIEW
        std::cout << "Gdk::Pixbuf rowstride " << pixbuf->get_rowstride()
                  << " bits " << pixbuf->get_bits_per_sample() << " chan "
                  << pixbuf->get_n_channels() << std::endl;
#endif
        auto dest = pixbuf->get_pixels();
        auto src = &m_memimg->data;
        const size_t rawRowSize{
                static_cast<unsigned>(m_memimg->width * m_memimg->colors)};
...
RawHelper::~RawHelper() { m_iProcessor.recycle(); }

void RawHelper::setBrightness(float bright) { m_bright = bright; }
void RawHelper::setContrast(float contrast) { m_contrast = contrast; }

// this gives nice looking 8bit-data
RawImageHelper RawHelper::read(const Glib::RefPtr<Gio::File> &file)
{
    // Let us create an image processor
    std::cout << "Open the file " << file->get_path()
              << " and read the metadata" << std::endl;
    ...
    // putenv((char *)"TZ=UTC"); // dcraw compatibility, affects TIFF datestamp
    // field
    ...
    if (ret != LIBRAW_SUCCESS) {
        std::cout << "Unable to dcraw_process " << libraw_strerror(ret)
                  << std::endl;
    ...
    if (!memimg) {
        std::cout << "error make_mem_image " << libraw_strerror(ret)
                  << std::endl;
...
RawHelper::readPreview(const Glib::RefPtr<Gio::File> &file)
{
    ...
    for (int t = 0; t < m_iProcessor.imgdata.thumbs_list.thumbcount; t++) {
        if ((ret = m_iProcessor.unpack_thumb_ex(t)) == LIBRAW_SUCCESS) {
            std::cout << "Got thumb " << t << " size "
                      << m_iProcessor.imgdata.thumbnail.twidth << "x"
                      << m_iProcessor.imgdata.thumbnail.theight << " fmt "
                      << (m_iProcessor.imgdata.thumbnail.tformat ==
                                          LIBRAW_THUMBNAIL_JPEG
                                  ? Glib::ustring("jpg")
                                  : Glib::ustring::sprintf(
                                            "fmt %d",
                                            m_iProcessor.imgdata.thumbnail
                                                    .tformat))
                      << std::endl;
            if (m_iProcessor.imgdata.thumbnail.tformat ==
...
RawHelper::processRaw2(const Glib::RefPtr<Gio::File> &file)
{
    // Let us create an image processor
#ifdef DEBUG_PREVIEW
    std::cout << "RawHelper::processRaw2 Open the file " << file->get_path()
              << " and read the metadata" << std::endl;
#endif
    //
    ...
    std::cout << "Image size: " << m_iProcessor.imgdata.sizes.width << " x "
              << m_iProcessor.imgdata.sizes.height << "\n"
    ...
    // iProcessor.adjust_maximum(); // does not change anything, tested various
    // locations
    ...
    for (size_t r = 0; r < m_iProcessor.imgdata.sizes.iheight; r++) {
        ...
        for (size_t c = 0; c < m_iProcessor.imgdata.sizes.iwidth; c++) {
            ...
            pixelMax = std::max(
                    pixelMax,
                    std::max(
                            m_iProcessor.imgdata.image[s][RAW_RED],
                            std::max(m_iProcessor.imgdata.image[s][RAW_GREEN],
                                     std::max(m_iProcessor.imgdata
                                                      .image[s][RAW_BLUE],
                                              m_iProcessor.imgdata
                                                      .image[s][RAW_GREEN2]))));
        }
    }
    float scale = 255.0f / static_cast<float>(pixelMax);
#ifdef DEBUG_PREVIEW
    std::cout << "pixel max " << pixelMax << " scale " << scale << std::endl;
#endif
    ...
    // the class
    auto pixbuf = Gdk::Pixbuf::create(Gdk::COLORSPACE_RGB, false, 8,
                                      m_iProcessor.imgdata.sizes.iwidth,
                                      m_iProcessor.imgdata.sizes.iheight);
#ifdef DEBUG_PREVIEW
    std::cout << "Gdk::Pixbuf rowstride " << pixbuf->get_rowstride() << " bits "
              << pixbuf->get_bits_per_sample() << " chan "
              << pixbuf->get_n_channels() << std::endl;
#endif
    ...
    for (size_t r = 0; r < m_iProcessor.imgdata.sizes.iheight; r++) {
        ...
        for (size_t c = 0; c < m_iProcessor.imgdata.sizes.iwidth; c++) {
            ...
            pixels[d + 0] = static_cast<uint8_t>(
                    (m_iProcessor.imgdata.image[s][RAW_RED] * scale));
            pixels[d + 1] = static_cast<uint8_t>(
                    (m_iProcessor.imgdata.image[s][RAW_BLUE] * scale));
            pixels[d + 2] = static_cast<uint8_t>(
                    (m_iProcessor.imgdata.image[s][RAW_GREEN] +
                     m_iProcessor.imgdata.image[s][RAW_GREEN2]) *
                    scale_2);
        }
    }
    return pixbuf;
...
RawImagePreview::RawImagePreview(BaseObjectType *cobject,
...
bool RawImagePreview::update_preview(const Glib::RefPtr<Gio::File> &rawFile)
{
    RawHelper rawHelper;
...
void RawImagePreview::reset_preview() { m_image->clear(); }

float RawImagePreview::getBrightness()
{
...
float RawImagePreview::getContrast()
{
...
void RawImageReader::prepareFilter(const Glib::RefPtr<Gtk::FileFilter> &filter)
{
    if (m_types.empty()) {
    ...
    filter->add_custom(Gtk::FileFilterFlags::FILE_FILTER_FILENAME,
                       sigc::mem_fun(*this, &RawImageReader::acceptFile));
}

void RawImageReader::update_preview()
{
...
void RawImageReader::addCustomPreview(Gtk::FileChooser &chooser,
                                      ApplicationSupport &appSupport)
{
    ...
    try {
        builder->add_from_resource(
                appSupport.getApplication()->get_resource_base_path() +
                "/raw-dlg.ui");
        ...
        chooser.signal_update_preview().connect(
                sigc::mem_fun(*this, &RawImageReader::update_preview));
    } catch (const Glib::Error &ex) {
        std::cout << Glib::ustring::sprintf("Unable to load raw-dlg: %s",
                                            ex.what())
                  << std::endl;
    }
}
...
Glib::RefPtr<Gdk::Pixbuf>
RawImageReader::read(const Glib::RefPtr<Gio::File> &file,
                     ImageFileChooser &file_chooser)
{
    bool accept{false};
    auto fileExt = StringUtils::lowerStd(
            StringUtils::getExtension(file->get_basename()));
    for (auto &ext : rawExtensions()) {
        if (ext == fileExt) {
            accept = true;
            break;
    ...
    if (accept) {
        ...
        if (rawPreview) {
            ...
            std::cout << "RawImageReader::read using "
                      ...
                      << " contrast " << rawPreview->getContrast() << "file "
...
#ifdef DEBUG_PREVIEW
    std::cout << "RawImageReader::read not accepted " << file->get_path()
              << std::endl;
#endif
    return Glib::RefPtr<Gdk::Pixbuf>();
}

bool RawImageReader::acceptFile(const Gtk::FileFilter::Info &filter_info)
{
