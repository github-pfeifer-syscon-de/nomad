...
void
X11Capture::blank_rectangle_in_pixbuf (GdkPixbuf *pixbuf, GdkRectangle *rect)
{
  int x, y;
  int x2, y2;
  guchar *pixels;
...
void
X11Capture::blank_region_in_pixbuf (GdkPixbuf *pixbuf, cairo_region_t *region)
{
  int n_rects;
  int i;
  int width, height;
...
cairo_region_t *
X11Capture::make_region_with_monitors (GdkDisplay *display)
{
 ...
 */
void
X11Capture::mask_monitors (GdkPixbuf *pixbuf,
...
void
X11Capture::get_window_rect_coords (GdkWindow    *window,
...
gboolean
X11Capture::window_is_desktop (GdkWindow *window)
{
...
Window
X11Capture::find_wm_window (GdkWindow *window)
{
...
GdkWindow*
X11Capture::do_find_current_window()
{
  GdkWindow *current_window;
  GdkDevice *device;
  GdkSeat *seat;

  current_window = find_active_window();
...
GdkWindow *
X11Capture::fallback_find_current_window ()
{
  GdkWindow *window = NULL;

  if (m_take_window_shot)
    {
      window = do_find_current_window ();

      if (window == NULL)
        m_take_window_shot = FALSE;
    }
...
GdkPixbuf *
X11Capture::get_pixbuf (GdkRectangle* rectangle)
{
  ...
  window = fallback_find_current_window();

  get_window_rect_coords(window,
                        &real_coords,
                        &screenshot_coords);
  ...
  if (wm != None)
    {
      ...
      get_window_rect_coords (wm_window,
                             &wm_real_coords,
                             NULL);

      frame_offset.left   = (gint16)(real_coords.x - wm_real_coords.x);
      frame_offset.top    = (gint16)(real_coords.y - wm_real_coords.y);
      frame_offset.right  = (gint16)(wm_real_coords.width - real_coords.width - frame_offset.left);
      frame_offset.bottom = (gint16)(wm_real_coords.height - real_coords.height - frame_offset.top);
    }
...
G_GNUC_END_IGNORE_DEPRECATIONS
   ...
   * screenshot */
  if (m_include_pointer && !rectangle)
    {
